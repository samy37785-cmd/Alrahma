...
async function main() {
  ...
  // -------------------------------------------------------------------
  await test("phase 3: apply the full migrations folder — only 0004-0017 actually run", async () => {
    ...
    // 0016 (parent linking + reviews_public view) and 0017 (PayPal
    // subscription-grant RPC) — 18 migrations total in the folder now, up
    // from 16. This count tracks the folder's real size, not a number to
    ...
    assert(applied.rows[0].n === expected, `expected all ${expected} migrations tracked as applied (0000-0003 recognized by journal timestamp, not hash + 0004-0017 newly run), got ${applied.rows[0].n}`);
  });
