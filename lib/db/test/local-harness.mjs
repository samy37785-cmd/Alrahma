 ...
 * `email`/`raw_user_meta_data`/`raw_app_meta_data` up front (the real
 * project's auth.users already has all three; our own auth.ts Drizzle
 * stub stays intentionally minimal — just `id`, enough for FK typing) so
 * handle_new_user() (0001) is testable immediately, with no separate
 * ALTER step needed after. `raw_app_meta_data` was added in PR #70 round
 * 9 (item 2) alongside migrate-users-to-supabase-auth.mjs moving
 * migration_correlation_id there (service-role-only, unlike
 * raw_user_meta_data which the account owner can write themselves).
 *
