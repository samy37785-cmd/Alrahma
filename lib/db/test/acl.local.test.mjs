...
async function main() {
  ...
  // Booking-First Enrollment: none of the 7 new admin/financial bookkeeping
  // columns are in the guest INSERT grant at all — anon is denied at the
  // GRANT layer for these, not merely filtered by a WITH CHECK, since the
  // real guest path is submit_enrollment_booking() (a SECURITY DEFINER RPC
  // whose parameter list has no way to pass any of them — see
  // 0025_booking_first_enrollment.sql), never a raw table INSERT.
  // (`authenticated` DOES have a column grant here — 0018's
  // enrollments_insert_admin_aal2 support needed an unrestricted INSERT
  // grant for a genuine AAL2 admin; a non-admin authenticated caller is
  // stopped by that policy's WITH CHECK at the RLS layer instead, proven
  // separately in rls.local.test.mjs, not by the GRANT.)
  await test("anon: none of enrollments' 7 new admin/financial columns are INSERT-grantable", async () => {
    for (const col of ["booking_ref", "agreed_amount", "currency", "payment_method_external", "paid_at", "renewal_at", "admin_note"]) {
      await assertColumn("anon", "enrollments", col, "INSERT", false, `${col} must never be forgeable by a guest`);
    }
  });
  ...
  // -------------------------------------------------------------------
  // Stage 2J-B / 0022_lossless_migration_support.sql — migration_source_
  // ledger / payment_source_snapshots. Not folded into ALL_TABLES above
  // (that constant is this file's original Round 1-4 table set, later
  // extended only for notifications/notification_preferences/
  // admin_audit_log): these two are pure migration-tooling bookkeeping,
  // never end-user- or admin-UI-facing, so they get their own precise
  // has_table_privilege() check rather than being swept into the
  // "authenticated: SELECT on every one of the N tables" assertion,
  // which would be actively wrong for them (they must have NO grant at
  // all, for any of anon/authenticated/service_role's raw table
  // privilege — service_role reaches them only via BYPASSRLS +ALSO the
  // explicit GRANT, both true, but PUBLIC's blanket REVOKE here is what
  // this test actually verifies).
  // -------------------------------------------------------------------
  await test("migration_source_ledger / payment_source_snapshots / email_outbox: PUBLIC/anon/authenticated have NO table privilege at all; service_role has all 4", async () => {
    // email_outbox (0029_email_outbox.sql, corrective revision to the
    // scope-correction pass) is server bookkeeping for the booking-
    // notification retry job — same "no anon/authenticated path at all"
    // shape as the two migration-tooling tables it's folded in with here.
    for (const table of ["migration_source_ledger", "payment_source_snapshots", "email_outbox"]) {
      for (const role of ["public", "anon", "authenticated"]) {
        for (const priv of ["SELECT", "INSERT", "UPDATE", "DELETE"]) {
          await assertTable(role, table, priv, false, `${table} is migration-tooling-only — ${role} must never have ${priv}`);
        }
      }
      for (const priv of ["SELECT", "INSERT", "UPDATE", "DELETE"]) {
        await assertTable("service_role", table, priv, true, `service_role is the only role meant to reach ${table}`);
      }
    }
  });
