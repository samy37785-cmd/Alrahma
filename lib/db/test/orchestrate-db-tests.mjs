...
async function main() {
  ...
  const runTests = async () => {
    const steps = [
      ...
      {
        name: "rpc-admin-booking-activation.local.test.mjs",
        run: () =>
          logged(
            "node test/rpc-admin-booking-activation.local.test.mjs",
            runNode(stepScript("test/rpc-admin-booking-activation.local.test.mjs"), { env: { ...process.env, TEST_DATABASE_URL: testDbUrl } }),
          ),
      },
      {
        name: "enrollment-status-reconciliation.local.test.mjs",
        run: () =>
          logged(
            "node test/enrollment-status-reconciliation.local.test.mjs",
            runNode(stepScript("test/enrollment-status-reconciliation.local.test.mjs"), { env: { ...process.env, TEST_DATABASE_URL: testDbUrl } }),
          ),
      },
      {
        name: "enrollment-new-to-pending-migration.local.test.mjs",
        run: () =>
          logged(
            "node test/enrollment-new-to-pending-migration.local.test.mjs",
            runNode(stepScript("test/enrollment-new-to-pending-migration.local.test.mjs"), { env: { ...process.env, TEST_DATABASE_URL: testDbUrl } }),
          ),
      },
    ];
