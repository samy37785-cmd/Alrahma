...
async function main() {
  ...
  // -------------------------------------------------------------------
  await test("phase 3: apply the full migrations folder — only 0004-0015 actually run", async () => {
    await migrate(db, { migrationsFolder: realMigrationsFolder });

    // Stage 2F added 0012-0015 (16 migrations total in the folder now,
    // up from 12) — this count tracks the folder's real size, not a
    ...
    assert(applied.rows[0].n === expected, `expected all ${expected} migrations tracked as applied (0000-0003 recognized by journal timestamp, not hash + 0004-0015 newly run), got ${applied.rows[0].n}`);
  });
  ...
  // -------------------------------------------------------------------
  await test("phase 6 (schema): exactly 36 tables in public", async () => {
    // 20-table baseline + 16 added by Stage 2F (0012/0013) — see
    ...
    assert(rows[0].n === 36, `expected 36 tables, got ${rows[0].n}`);
  });
