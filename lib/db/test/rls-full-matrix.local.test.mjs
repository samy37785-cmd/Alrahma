...
async function main() {
  ...
  let guestPaymentId, ledgerId, paymentSourceSnapshotId;
  ...
  await test("seed: all fixtures", async () => {
    ...
    // Stage 2J-B / 0022_lossless_migration_support.sql — a genuinely
    // unlinked ("guest") charge: user_id IS NULL, identified only by its
    // snapshot columns. docs/rls-matrix.md row 8, note 18.
    guestPaymentId = (await client.query(
      `insert into public.payments (user_id, amount_minor, gateway, customer_name_snapshot, customer_email_snapshot)
       values (NULL, 2000, 'stripe', 'Matrix Guest', 'matrix-guest@example.test') returning id;`,
    )).rows[0].id;
    await client.query(`update public.payments set status = 'succeeded' where id = $1;`, [guestPaymentId]);

    // docs/rls-matrix.md rows 21-22, note 19 — service_role-only tooling
    // bookkeeping, never end-user- or admin-UI-facing.
    ledgerId = (await client.query(
      `insert into public.migration_source_ledger
         (source_system, source_database, source_collection, source_document_id, source_content_hash, target_table, target_id, status)
       values ('mongo', 'al-rahma', 'payments', 'matrix-fixture-doc-1', 'matrixhash1', 'payments', $1, 'reconciled')
       returning id;`,
      [String(chargeId)],
    )).rows[0].id;
    paymentSourceSnapshotId = (await client.query(
      `insert into public.payment_source_snapshots (payment_id, source_system, source_collection, source_document_id, raw_payload)
       values ($1, 'mongo', 'payments', 'matrix-fixture-doc-1', '{"gateway":"stripe"}'::jsonb)
       returning id;`,
      [chargeId],
    )).rows[0].id;
  ...
  // ===================================================================
  // Stage 2J-B / 0022 — docs/rls-matrix.md row 8, note 18: a guest
  // (user_id IS NULL) charge is invisible to every authenticated user
  // (there is no owner to match `auth.uid()` against) and reachable only
  // through is_admin_aal2(), exactly like another user's linked payment.
  await test("[payments] guest (user_id IS NULL) payment is invisible to anon and to any authenticated user, but AAL2 admin CAN read it", async () => {
    await asAnon();
    await expectReject(() => client.query(`select * from public.payments where id = $1;`, [guestPaymentId]));

    await asUser(userAId);
    const asOwnerLike = await client.query(`select * from public.payments where id = $1;`, [guestPaymentId]);
    assert(asOwnerLike.rows.length === 0, "a guest payment must not be visible to a plain authenticated user");

    await asUser(adminId, "aal1");
    const aal1 = await client.query(`select * from public.payments where id = $1;`, [guestPaymentId]);
    assert(aal1.rows.length === 0, "AAL1 admin needs AAL2 to read payments, guest or not");

    await asUser(adminId, "aal2");
    const aal2 = await client.query(`select * from public.payments where id = $1;`, [guestPaymentId]);
    assert(aal2.rows.length === 1, "AAL2 admin should be able to read a guest payment");
    assert(aal2.rows[0].user_id === null, "the row must genuinely have user_id NULL, not a fabricated profile");
  });
  ...
  // -------------------------------------------------------------------
  // Production-readiness audit follow-up (2026-09-17): increment_blog_view()
  // (0034_blog_view_increment.sql) closes the "view count doesn't
  // increment under DATA_BACKEND=supabase" gap — a narrowly-scoped
  // SECURITY DEFINER RPC granted to anon that only increments views on an
  // already-published post matched by slug, nothing else. Proves it here
  // (not just read as SQL) against the real RLS/GRANT layer.
  // -------------------------------------------------------------------
  await test("[increment_blog_view] anon can increment a published post's view count by exactly 1 per call", async () => {
    await asAnon();
    const before = await client.query(`select views from public.blogs where id = $1;`, [blogId]);
    const res1 = await client.query(`select public.increment_blog_view($1) as views;`, ["matrix-published"]);
    assert(res1.rows[0].views === before.rows[0].views + 1, "first call should increment by exactly 1");
    const res2 = await client.query(`select public.increment_blog_view($1) as views;`, ["matrix-published"]);
    assert(res2.rows[0].views === before.rows[0].views + 2, "second call should increment by exactly 1 more");
  });
  await test("[increment_blog_view] does NOT increment a draft (unpublished) post, and returns null rather than an error", async () => {
    await asSuperuser();
    const before = await client.query(`select views from public.blogs where id = $1;`, [draftBlogId]);

    await asAnon();
    const res = await client.query(`select public.increment_blog_view($1) as views;`, ["matrix-draft"]);
    assert(res.rows[0].views === null, "must return null for a slug matching only an unpublished post");

    // Checked as superuser, not anon — RLS correctly hides the draft from
    // anon entirely (proven by the adjacent "anon SELECT published, not
    // the draft" test above), which is a distinct fact from "the RPC
    // didn't increment it"; using anon here would just re-prove the RLS
    // read-visibility test and mask whether the row itself actually
    // changed underneath.
    await asSuperuser();
    const after = await client.query(`select views from public.blogs where id = $1;`, [draftBlogId]);
    assert(after.rows[0].views === before.rows[0].views, "the draft's view count must be unchanged");
  });
  await test("[increment_blog_view] a nonexistent slug returns null, no error, nothing created", async () => {
    await asAnon();
    const res = await client.query(`select public.increment_blog_view($1) as views;`, ["matrix-slug-does-not-exist"]);
    assert(res.rows[0].views === null);
  });
  ...
  // ===================================================================
  // 21-22. migration_source_ledger / payment_source_snapshots —
  // docs/rls-matrix.md rows 21-22, note 19. Pure migration-tooling
  // bookkeeping: REVOKE ALL FROM PUBLIC, anon, authenticated,
  // service_role then GRANT ALL TO service_role only, RLS enabled with
  // NO policy at all — anon/authenticated (owner, other, AAL1, AAL2
  // admin alike, since admin sessions are still Postgres role
  // `authenticated`) are denied at the GRANT layer, before RLS is even
  // reached; only service_role (via BYPASSRLS + the explicit grant)
  // reaches either table.
  // ===================================================================
  await test("[migration_source_ledger] anon denied at the GRANT layer", async () => {
    await asAnon();
    await expectReject(() => client.query(`select * from public.migration_source_ledger where id = $1;`, [ledgerId]));
  });
  await test("[migration_source_ledger] a plain authenticated user denied at the GRANT layer", async () => {
    await asUser(userAId);
    await expectReject(() => client.query(`select * from public.migration_source_ledger where id = $1;`, [ledgerId]));
  });
  await test("[migration_source_ledger] AAL2 admin ALSO denied at the GRANT layer — this is not admin-UI-facing data", async () => {
    await asUser(adminId, "aal2");
    await expectReject(() => client.query(`select * from public.migration_source_ledger where id = $1;`, [ledgerId]));
  });
  await test("[migration_source_ledger] service_role CAN read/write it", async () => {
    await asService();
    const { rows } = await client.query(`select * from public.migration_source_ledger where id = $1;`, [ledgerId]);
    assert(rows.length === 1, "service_role should be able to read the ledger");
    await client.query(`update public.migration_source_ledger set status = 'reconciled' where id = $1;`, [ledgerId]);
  });

  await test("[payment_source_snapshots] anon denied at the GRANT layer", async () => {
    await asAnon();
    await expectReject(() => client.query(`select * from public.payment_source_snapshots where id = $1;`, [paymentSourceSnapshotId]));
  });
  await test("[payment_source_snapshots] the OWNER of the payment itself still cannot read its raw snapshot — no owner path exists at all", async () => {
    await asUser(userAId);
    await expectReject(() => client.query(`select * from public.payment_source_snapshots where id = $1;`, [paymentSourceSnapshotId]));
  });
  await test("[payment_source_snapshots] AAL2 admin ALSO denied at the GRANT layer — customer/raw payload never reaches even the admin UI's normal session", async () => {
    await asUser(adminId, "aal2");
    await expectReject(() => client.query(`select * from public.payment_source_snapshots where id = $1;`, [paymentSourceSnapshotId]));
  });
  await test("[payment_source_snapshots] service_role CAN read it", async () => {
    await asService();
    const { rows } = await client.query(`select * from public.payment_source_snapshots where id = $1;`, [paymentSourceSnapshotId]);
    assert(rows.length === 1, "service_role should be able to read the raw snapshot");
  });
