...
async function main() {
  ...
  // -------------------------------------------------------------------
  await test("phase 3: apply the full migrations folder — only 0004-0010 actually run", async () => {
    ...
    assert(applied.rows[0].n === 11, `expected all 11 migrations tracked as applied (0000-0003 recognized by journal timestamp, not hash + 0004-0010 newly run), got ${applied.rows[0].n}`);
  });
