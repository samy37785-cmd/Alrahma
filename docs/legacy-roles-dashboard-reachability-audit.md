...
## 25. Status note — Stage 2C Final Corrective execution

A follow-up task closed three account-role gaps Stage 2C's own closing document (`docs/legacy-role-orphan-cleanup.md`) had explicitly left as UNKNOWN or reviewed-and-kept rather than resolved: `AdminUsersTab.jsx`'s raw role display and family-name field (both deleted as orphans), `AdminDashboard.jsx`'s role-derived KPIs and "Teachers list" card (deleted, replaced with subscription-based KPIs), and `AdminClassesTab.jsx`'s `role === 'student'` eligibility filter (fixed to use the full regular-user list). It also applied `normalizeAccountRole()` at `AuthContext`'s real data boundaries for the first time and redesigned the Admin session to be fail-closed. Full detail: `docs/user-admin-auth-contract.md`, Appendix B.
