...
async function main() {
  ...
  // -------------------------------------------------------------------
  await test("phase 6 (schema): exactly 39 tables in public", async () => {
    // 20-table baseline + 16 added by Stage 2F (0012-0015) + 1 added by
    // 0016 (parent_student_links) + 2 added by Stage 2J-B / 0022
    // (migration_source_ledger, payment_source_snapshots) — see
    // schema.local.test.mjs's matching test for the full name list.
    ...
    assert(rows[0].n === 39, `expected 39 tables, got ${rows[0].n}`);
  });
