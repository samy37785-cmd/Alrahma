...
// `finally`. It does NOT count toward the DB suite's own assertion total
// (DB_ASSERTION_CONTRACT_TOTAL, orchestrator-lib.mjs — 253 as of Booking-
// First Enrollment's 0025 migration)
...
// log.
//
// Four things are proven here, using orchestrator-lib.mjs's and
// orchestrate-db-tests.mjs's REAL, unmodified exports (never a
...
//   4. interpretDockerPsResult()/cleanupDisposablePostgres() — the
//      fail-CLOSED presence check added in Stage 0 Final Corrective to
//      close the "a failed docker ps could be read as verified absence"
//      finding: a nonzero exit code or a thrown error from `docker ps`
//      must classify as unknown (null), never as false ("absent") —
//      proven both directly against the pure classifier and against the
//      real cleanup function with a fake docker-command runner injected
//      in place of real Docker.
...
import { checkContainerPresence, cleanupDisposablePostgres } from "./orchestrate-db-tests.mjs";
...
/**
 * A fake `runCommand`-shaped function (same `(command, args) =>
 * Promise<{code, stdout, stderr}>` contract as orchestrator-lib.mjs's real
 * runCommand()) for injecting into cleanupDisposablePostgres() without
 * touching real Docker. `plan.stop`/`plan.rm` are each either a
 * `{code, stdout, stderr}` result or an `Error` instance to throw.
 * `plan.ps` is an ARRAY of such results/Errors, consumed in call order —
 * the first `docker ps` call gets `plan.ps[0]` (the "initial" presence
 * check), the second gets `plan.ps[1]` (the "final" check, whether or not
 * an `rm -f` fallback ran in between).
 */
function makeFakeDockerRunner(plan) {
  let psCallIndex = 0;
  return async (command, args) => {
    assert(command === "docker", `fake runner only understands "docker", got "${command}"`);
    const sub = args[0];
    let entry;
    if (sub === "stop") entry = plan.stop;
    else if (sub === "rm") entry = plan.rm;
    else if (sub === "ps") entry = plan.ps[psCallIndex++];
    else throw new Error(`fake docker runner: unexpected subcommand "${sub}"`);
    if (entry === undefined) throw new Error(`fake docker runner: no plan entry for "${sub}" (call #${psCallIndex})`);
...
async function main() {
  ...
  // -----------------------------------------------------------------
  await test("contract: the fully correct assertion set (DB_ASSERTION_CONTRACT_TOTAL) satisfies the contract", async () => {
    const steps = [
      ...goldenSteps(),
  ...
  await test("contract: a suite reporting the wrong total (e.g. 1/1 instead of its real expected count) fails the gate", async () => {
    const steps = goldenSteps();
    const victim = steps.find((s) => s.name === "acl.local.test.mjs"); // really expects 20
    victim.passed = 1;
    victim.total = 1;
    const result = evaluateAssertionContract(steps);
    assert(result.ok === false, "expected ok=false for a suite reporting the wrong total");
    assert(
      result.problems.some((p) => p.includes("acl.local.test.mjs") && p.includes("reported total 1, expected exactly 20")),
      `expected a wrong-total problem for acl.local.test.mjs, got: ${JSON.stringify(result.problems)}`,
    );
  });

  await test("contract: aggregate below DB_ASSERTION_CONTRACT_TOTAL fails the gate even though every PRESENT suite has passed === total", async () => {
    ...
    assert(result.aggregate.total < DB_ASSERTION_CONTRACT_TOTAL, "expected the aggregate to fall short of the contract total");
  });
  ...
  // -----------------------------------------------------------------
  // Part 4 — Docker presence check must fail CLOSED (Stage 0 Final
  // Corrective). An earlier cleanupDisposablePostgres() collapsed a
  // FAILED `docker ps` (nonzero exit, or a thrown error) into the same
  // `false` ("absent") result as a genuinely successful check that found
  // nothing — meaning a failed presence check could look exactly like
  // verified absence. Proven here two ways: (a) directly, against the
  // pure interpretDockerPsResult() classifier, with zero Docker/child-
  // process involvement; (b) against the REAL cleanupDisposablePostgres()
  // (imported from orchestrate-db-tests.mjs, never a re-implementation),
  // with a fake docker-command runner injected in its place — no real
  // Docker touched, but every branch of the real cleanup logic exercised.
  // -----------------------------------------------------------------

  await test("presence: docker ps exit 0 + output has the container name → present = true", async () => {
    const result = interpretDockerPsResult({ code: 0, stdout: "my-container\n" }, "my-container");
    assert(result.presence === true, `expected presence=true, got ${result.presence}`);
    assert(result.exitCode === 0, "expected exitCode 0");
  });

  await test("presence: docker ps exit 0 + empty output → present = false", async () => {
    const result = interpretDockerPsResult({ code: 0, stdout: "" }, "my-container");
    assert(result.presence === false, `expected presence=false, got ${result.presence}`);
    assert(result.exitCode === 0, "expected exitCode 0");
  });

  await test("presence: docker ps exits NONZERO → unknown (null), never false", async () => {
    const result = interpretDockerPsResult({ code: 1, stdout: "" }, "my-container");
    assert(result.presence === null, `expected presence=null (unknown), got ${result.presence} — a failed check must never read as "absent"`);
    assert(result.exitCode === 1, "expected exitCode 1 to be preserved for logging");
  });

  await test("presence: docker ps THROWS (command could not be run at all) → unknown (null)", async () => {
    const result = interpretDockerPsResult(null, "my-container"); // null = "the command threw, there is no result"
    assert(result.presence === null, `expected presence=null (unknown), got ${result.presence}`);
  });

  await test("cleanup: a nonzero FINAL presence check makes verified=false, even if stop and the initial check looked fine", async () => {
    const runner = makeFakeDockerRunner({
      stop: { code: 0, stdout: "", stderr: "" },
      ps: [
        { code: 0, stdout: "my-container\n", stderr: "" }, // initial: present=true → rm attempted
        { code: 1, stdout: "", stderr: "docker daemon busy" }, // final: FAILS → unknown, never "absent"
      ],
      rm: { code: 0, stdout: "", stderr: "" },
    });
    const outcome = await cleanupDisposablePostgres("my-container", runner);
    assert(outcome.finalCheckExitCode === 1, "expected the real final check exit code to be preserved");
    assert(outcome.finalPresence === null, "expected finalPresence=null (unknown), not false");
    assert(outcome.verified === false, "a failed final presence check must never be reported as verified");
  });

  await test("lifecycle: a genuinely successful test run plus a FAILED final docker ps check still blocks evidence write", async () => {
    const runner = makeFakeDockerRunner({
      stop: { code: 0, stdout: "", stderr: "" },
      ps: [
        { code: 0, stdout: "", stderr: "" }, // initial: absent already → no rm attempted
        { code: 1, stdout: "", stderr: "connection reset" }, // final: FAILS
      ],
    });
    let writeCalls = 0;
    const outcome = await runLifecycle({
      ...
      cleanup: () => cleanupDisposablePostgres("my-container", runner),
      writeEvidenceIfGreen: async (o) => {
        if (o.fullyGreen) writeCalls++;
      },
    });
    assert(outcome.cleanupResult.verified === false, "expected cleanup.verified=false when the final check itself failed");
    assert(outcome.fullyGreen === false, "expected the overall run to be NOT fully green");
    assert(writeCalls === 0, "evidence must NOT have been written when the final presence check failed");
  });

  await test("cleanup: docker stop fails, but an independent final check confirms genuine absence → verified=true (the final confirmed state wins)", async () => {
    const runner = makeFakeDockerRunner({
      stop: { code: 1, stdout: "", stderr: "Error: No such container (already gone)" }, // stop itself "fails"
      ps: [
        { code: 0, stdout: "", stderr: "" }, // initial: genuinely absent already → no rm needed
        { code: 0, stdout: "", stderr: "" }, // final: independently confirms absence
      ],
    });
    const outcome = await cleanupDisposablePostgres("my-container", runner);
    assert(outcome.stopExitCode === 1, "expected the real (failing) stop exit code to be preserved for the record");
    assert(outcome.rmAttempted === false, "no rm fallback should have been attempted — initial check already confirmed absence");
    assert(outcome.finalPresence === false, "expected the final check to confirm absence");
    assert(outcome.verified === true, "a failed docker stop must not matter when the final independent check proves the container is gone");
  });

  await test("cleanup: docker rm -f fails, and the final check confirms the container is STILL present → verified=false", async () => {
    const runner = makeFakeDockerRunner({
      stop: { code: 0, stdout: "", stderr: "" },
      ps: [
        { code: 0, stdout: "my-container\n", stderr: "" }, // initial: present=true → rm attempted
        { code: 0, stdout: "my-container\n", stderr: "" }, // final: STILL present after rm failed
      ],
      rm: { code: 1, stdout: "", stderr: "Error: cannot remove container: still running" },
    });
    const outcome = await cleanupDisposablePostgres("my-container", runner);
    assert(outcome.rmAttempted === true, "expected the rm -f fallback to have been attempted");
    assert(outcome.rmExitCode === 1, "expected the real failing rm exit code to be preserved");
    assert(outcome.finalPresence === true, "expected the final check to confirm the container is still there");
    assert(outcome.verified === false, "verified must be false when the final check shows the container is still present");
  });

  await test("cleanup: the fully successful path (stop ok, initial present, rm ok, final absent) is verified=true", async () => {
    const runner = makeFakeDockerRunner({
      stop: { code: 0, stdout: "", stderr: "" },
      ps: [
        { code: 0, stdout: "my-container\n", stderr: "" },
    ...
    const outcome = await cleanupDisposablePostgres("my-container", runner);
    assert(outcome.verified === true, "expected the ordinary fully-successful cleanup path to verify");
  });
