...
async function main() {
  ...
  // -------------------------------------------------------------------
  await test("phase 3: apply the full migrations folder — only 0004-0021 actually run", async () => {
    ...
    // route-parity gap), and 0021 (service_role SELECT grant on
    // auth.users/auth.mfa_factors, found by the real-GoTrue auth-migration
    // rehearsal — see docs/option-a-mongo-supabase-parity-map.md) — 22
    // migrations total in the folder now, up from 21. This count tracks
    ...
    assert(applied.rows[0].n === expected, `expected all ${expected} migrations tracked as applied (0000-0003 recognized by journal timestamp, not hash + 0004-0020 newly run), got ${applied.rows[0].n}`);
  });
  ...
  // -------------------------------------------------------------------
  await test("phase 6 (schema): exactly 37 tables in public", async () => {
    // 20-table baseline + 16 added by Stage 2F (0012-0015) + 1 added by
    // 0016 (parent_student_links) — see schema.local.test.mjs's matching
    // test for the full name list.
    ...
    assert(rows[0].n === 37, `expected 37 tables, got ${rows[0].n}`);
  });
