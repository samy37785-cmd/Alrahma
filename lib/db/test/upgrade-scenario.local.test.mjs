...
async function main() {
  ...
  // -------------------------------------------------------------------
  await test("phase 3: apply the full migrations folder — only 0004-0024 actually run", async () => {
    ...
    // longest_streak/last_*_date columns), and 0023 (PR #70 review round
    ...
    // partial unique index), and 0024 (PR #70 review round 9, item 4:
    // unifies the 'failed'-row policy by dropping and re-creating 0023's
    // own partial unique index to also cover status='failed') — 25
    // migrations total in the folder now, up from 24. This count tracks
    // the folder's real size, not a number to keep in sync by hand every
    // time a migration is added.
    const expected = 25;
    const applied = await pool.query(`select count(*)::int as n from drizzle.__drizzle_migrations;`);
    assert(applied.rows[0].n === expected, `expected all ${expected} migrations tracked as applied (0000-0003 recognized by journal timestamp, not hash + 0004-0021 newly run), got ${applied.rows[0].n}`);
  });
