...
async function main() {
  // -------------------------------------------------------------------
  // Table count: 36 in `public` — the original 20-table baseline (locked
  // per docs/product-scope-audit.md §12) plus 16 added by Stage 2F
  // (lib/db/drizzle/0012_new_domains_baseline.sql, 0013_admin_rbac.sql):
  // courses, course_progress, live_classes, messages, student_records,
  // hifz_progress, document_counters, certificates, reviews, referrals,
  // wishlists, contact_messages, system_config, admin_role_assignments,
  // role_permissions, user_extra_permissions. Stage 2F was an explicit,
  // user-directed reversal of §12's decision to keep 11 of those names out
  // of Postgres (see the updated `dropped` list below) — not a drift from
  // the locked baseline, a deliberate, separately-authorized expansion of
  // it, same as 0013's admin RBAC tables which never existed in either
  // version of the scope audit.
  // -------------------------------------------------------------------
  await test("public schema has exactly 40 tables (auth.users excluded)", async () => {
    // 36 (Stage 2F baseline) + 1: parent_student_links (0016 — closes the
    // parent<->child linking gap parentController.js's 501s documented).
    // 0016's reviews_public is a VIEW, not a BASE TABLE, so it does not
    // affect this count. + 2 (Stage 2J-B / 0022_lossless_migration_
    // support.sql): migration_source_ledger, payment_source_snapshots —
    // both new tables, no existing table dropped. + 1 (corrective revision
    // to the scope-correction pass, 0029_email_outbox.sql): email_outbox —
    // a new outbox/retry table for booking-notification emails, no
    ...
    assert(
      rows.length === 40,
      `expected 40 public tables, got ${rows.length}: ${rows.map((r) => r.table_name).join(", ")}`,
    );
  });

  await test("auth.users and auth.mfa_factors are the only non-public, non-drizzle-internal tables", async () => {
    ...
    // — expected here, excluded from this check. auth.mfa_factors is a
    // Stage 2F addition to local-harness.mjs's auth stub (lib/db/drizzle
    // never creates it — same discipline as auth.users itself): the real
    // Supabase project already has GoTrue's own MFA schema; this backend
    // only ever reads from it (data/supabase/loadAdmin.js), so a local
    // structural stand-in is needed to test that code path at all.
    const { rows } = await pool.query(`
      ...
      order by table_name;
    `);
    const names = rows.map((r) => `${r.table_schema}.${r.table_name}`).sort();
    assert(
      names.length === 2 && names[0] === "auth.mfa_factors" && names[1] === "auth.users",
      `expected only auth.users/auth.mfa_factors outside public/drizzle, got: ${JSON.stringify(rows)}`,
    );
  });

  await test("none of the 9 still-dropped tables exist", async () => {
    // Baseline remediation: this used to be a hand-guessed list of
    // plausible-sounding table names that did NOT match reality — it
    // never actually tested against the real DROP list. This was the
    // real, locked list from docs/product-scope-audit.md §12 (20 names);
    // Stage 2F reversed §12's decision for 11 of them (courses,
    // course_progress, certificates, student_records, live_classes,
    // hifz_progress, referrals, messages, reviews, system_config,
    // contact_messages — all 12 of Stage 2F's new domains except
    // `wishlists`, whose real name never matched the dropped
    // `wishlist_items` to begin with) by actually building them
    // (0012_new_domains_baseline.sql). The remaining 9 are still
    // genuinely out of scope and still checked here.
    const dropped = [
      "profile_children", "admin_lockouts", "posts", "post_likes", "comments",
      "wishlist_items", "tutor_conversations", "system_audit_log", "rate_limit_counters",
    ];
    assert(dropped.length === 9, `expected exactly 9 dropped-table names to check, got ${dropped.length}`);
    const { rows } = await pool.query(`
      select table_name from information_schema.tables
  ...
  // Stage 2J-B / 0022_lossless_migration_support.sql: payments.user_id ->
  // nullable. This is NOT just a migration convenience — it fixes a real
  // pre-existing bug. backend/data/supabase/stripeController.js and
  // paypalController.js are both documented "@access Public (softProtect
  // attaches req.user if logged in)" and already compute
  // `userId = req.user?._id ?? null` for a guest checkout — the live
  // backend has always intended to support an unauthenticated Stripe/
  // PayPal checkout. Before this migration, that code path's own INSERT
  // would have thrown on the DB's NOT NULL constraint the instant a real
  // guest actually checked out; this test used to assert that broken
  // behavior was correct. It was not — the DB was silently more
  // restrictive than the application code it was supposed to support.
  await test("payments insert without user_id now succeeds (guest checkout — Stage 2J-B / 0022 fixed a real NOT NULL bug blocking softProtect's Public checkout routes)", async () => {
    const { rows } = await pool.query(
      `insert into public.payments (plan_id, amount_minor, gateway) values ($1, 5000, 'stripe') returning id, user_id;`,
      [planId],
    );
    assert(rows[0].user_id === null, "a guest payment must genuinely have user_id NULL, not a fabricated profile");
  });

  await test("payments insert without user_id AND without a snapshot column is still allowed (snapshot columns are informational, not a substitute ownership key)", async () => {
    // Deliberately not asserting a NOT NULL/CHECK requirement here: RLS
    // (payments_select_own_or_admin_aal2) is what actually makes an
    // unidentified guest charge invisible to every non-admin regardless
    // of whether a snapshot was captured — see docs/rls-matrix.md note 18.
    await pool.query(
  ...
  // -------------------------------------------------------------------
  // STOP GATE C / 0026_live_classes_nullable_teacher.sql — schema-level
  // half of the real-DB proof (the RLS/functional half lives in
  // rls.local.test.mjs). An admin-scheduled live class has no regular-
  // user teacher identity to assign (AdminUser and profiles are separate
  // identities under this backend — see loadAdmin.js's module comment);
  // Mongo mode already made LiveClass.teacher optional/nullable for the
  // same reason, and the app-layer validation in
  // data/supabase/admin/liveClassesAdminController.js was relaxed to
  // match — but only THIS assertion proves the database itself actually
  // accepts a NULL there now, independent of whatever the application
  // layer allows through.
  // -------------------------------------------------------------------
  await test("0026: live_classes.teacher_id is nullable after the full migration journal (NOT NULL genuinely dropped, not just accepted by app-layer validation)", async () => {
    const { rows } = await pool.query(`
      select is_nullable from information_schema.columns
      where table_schema = 'public' and table_name = 'live_classes' and column_name = 'teacher_id';
    `);
    assert(rows.length === 1, "live_classes.teacher_id column not found");
    assert(rows[0].is_nullable === "YES", "live_classes.teacher_id must be nullable after 0026 — an admin-scheduled class has no teacher to assign");
  });
