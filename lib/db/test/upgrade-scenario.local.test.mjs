...
async function main() {
  ...
  // -------------------------------------------------------------------
  await test("phase 3: apply the full migrations folder — only 0004-0020 actually run", async () => {
    ...
    // (teachers_public view, for the /api/search route-parity gap), and
    // 0020 (subscriptions.renewal_reminder_sent_for, for the /api/cron
    // route-parity gap) — 21 migrations total in the folder now, up from
    // 20. This count tracks the folder's real size, not a number to keep in
    // sync by hand every time a migration is added.
    const expected = 21;
