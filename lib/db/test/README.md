...
- `test/rls.local.test.mjs` — 88 real-SQL assertions (specific findings: bypass closure, forgery prevention, AAL boundaries, concurrency, webhook lease + fencing, subscription/invoice/refund RPCs, Booking-First Enrollment's submit_enrollment_booking() RPC, 0026's live_classes nullable-teacher admin/student/non-admin/FK coverage, 0027's reviews_insert_own course_progress/my_teacher_id() relationship checks, 0033's bump_token_version() self-only/unauthenticated-rejected coverage)
...
**260 real-SQL/filesystem assertions total: 4 checksum + 69 schema + 88
...
not a mocked controller test. The auth hardening security batch's
0027_review_relationship_check.sql then raised it from 252 to 258: +6
targeted RLS (reviews_insert_own now requires a real course_progress row
for a course review or a my_teacher_id() match for a teacher review — a
student with the relationship succeeds, one without it is rejected, and
setting both course_id and teacher_id together to smuggle an owned
relationship alongside an unowned one is still rejected). Full production
cutover's 0033_profiles_token_version.sql then raised it from 258 to 260:
+2 targeted RLS (bump_token_version() only ever increments the calling
user's own row, never another's, starting from 0; an unauthenticated
caller is rejected). See each migration's own tests for what every new
assertion covers.) Neither
...
safe self-test — **not counted in the 260** — that proves, by really
...
2. The exact assertion contract (below) correctly fails the gate for
   ...
   and correctly passes for the one genuinely-correct 260/260 shape.
...
- `rls.local.test.mjs` = exactly 88/88
...
- the resulting aggregate = exactly **260/260**
...
1. the exact assertion contract above is satisfied (260/260, not a naive
   sum);
...
230-assertion suite (the contract's total at Stage 0 — now 260 after
Stage 2J-B, Booking-First Enrollment's 0025 migration, STOP GATE C's
0026 migration, the auth hardening security batch's 0027 migration, and
full production cutover's 0033 migration, see above) was run twice, each
