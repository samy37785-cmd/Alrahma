...
async function main() {
  ...
  let guestBookingRef;
  await test("submit_enrollment_booking(): anon gets back a real, well-formed booking_ref; status is forced to 'new'; no financial field is settable", async () => {
    await asAnon();
    ...
    assert(row.rows[0].status === "new", "status must be forced to 'new', never guest-suppliable");
