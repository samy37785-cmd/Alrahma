...
/**
 * Stops, verifies-absent, and (if needed) force-removes the disposable
 * container — capturing every real docker exit code along the way rather
 * than assuming success. Returns a structured outcome; NEVER throws (a
 * docker-command failure is captured in the outcome, not swallowed —
 * `verified` simply comes back false and the caller decides what that
 * means for the overall run).
 */
async function cleanupDisposablePostgres(containerName) {
  const outcome = {
    containerName,
    stopExitCode: null,
    stopStderr: "",
  ...
  if (!containerName) {
    ...
    outcome.verified = true; // vacuously true: there is genuinely nothing to be present
    outcome.containerStillPresent = false;
    return outcome;
  }
  ...
  const checkPresence = async () => {
    try {
      ...
      outcome.finalCheckExitCode = check.code;
      return check.code === 0 && check.stdout.trim() === containerName;
    } catch (err) {
      outcome.error = (outcome.error ? outcome.error + "; " : "") + `docker ps threw: ${err.message}`;
      outcome.finalCheckExitCode = null;
      return null; // unknown — cannot claim verified
    }
  };

  let stillPresent = await checkPresence();

  if (stillPresent === true) {
    try {
      const rmResult = await runCommand("docker", ["rm", "-f", containerName]);
    ...
    stillPresent = await checkPresence();
  }

  outcome.containerStillPresent = stillPresent;
  outcome.verified = stillPresent === false;
  return outcome;
}
...
async function main() {
  ...
  const cleanup = async () => {
    ...
    log(
      `docker stop exit=${cleanupResult.stopExitCode ?? "(not run)"}` +
        (cleanupResult.rmExitCode !== null ? `, docker rm -f exit=${cleanupResult.rmExitCode}` : "") +
        `, final docker ps check exit=${cleanupResult.finalCheckExitCode ?? "(not run)"}`,
    );
    if (cleanupResult.verified) {
      log(`cleanup verified: container absent`);
    } else {
      log(`cleanup NOT verified: container "${containerName}" may still be present.${cleanupResult.error ? ` (${cleanupResult.error})` : ""}`);
