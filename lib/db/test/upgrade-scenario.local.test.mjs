 ...
 * migrations folder (byte-identical file content AND identical
 * meta/_journal.json `when` timestamps for those 4 entries — it's the
 * timestamp match, not a hash comparison, that makes phase 3's full-
 * folder migrate() run recognize these as already applied; see that
 * phase's own comment), not a rewritten approximation. */
...
async function main() {
  ...
  // -------------------------------------------------------------------
  // Phase 3: apply the REAL, FULL migrations folder.
  //
  // Corrected description (a review caught this — the previous wording
  // claimed a hash comparison that doesn't actually happen): reading
  // drizzle-orm's own PgDialect.migrate() source shows it queries only
  // the SINGLE most-recently-applied row from __drizzle_migrations
  // (`order by created_at desc limit 1`), and runs every migration whose
  // meta/_journal.json timestamp (`when`, aka folderMillis) is newer
  // than that one row's created_at. It computes and inserts a sha256
  // hash per NEWLY-applied migration for bookkeeping, but never reads
  // that hash back to re-verify an OLDER, already-applied migration's
  // file content is still unchanged.
  //
  // So 0000-0003 are skipped here because phase 1's trimmed journal
  // (assembled above) carries the SAME `when` timestamps as the real
  // folder's 0000-0003 entries — that timestamp match, not a hash
  // comparison, is what this phase actually exercises. This proves the
  // real upgrade ORDERING (only the genuinely-new migrations run against
  // an already-partially-migrated database) — it does NOT prove 0000-
  // 0003's file content is tamper-evident; see
  // published-migrations-checksum.test.mjs for that separate guarantee.
  // -------------------------------------------------------------------
  await test("phase 3: apply the full migrations folder — only 0004+ actually run", async () => {
    await migrate(db, { migrationsFolder: realMigrationsFolder });

    // Stage 2F added 0012-0015; Part A of "Al-Rahma Final Corrections" added
    // 0016 (parent linking + reviews_public view), 0017 (PayPal
    // subscription-grant RPC), 0018 (admin subrouter closure), 0019
    // (teachers_public view, for the /api/search route-parity gap), 0020
    // (subscriptions.renewal_reminder_sent_for, for the /api/cron
    // route-parity gap), 0021 (service_role SELECT grant on
    // auth.users/auth.mfa_factors, found by the real-GoTrue auth-migration
    // rehearsal — see docs/option-a-mongo-supabase-parity-map.md), 0022
    // (Stage 2J-B lossless-migration support: nullable payments.user_id +
    // customer/gateway-order snapshot columns, the migration_source_ledger
    // and payment_source_snapshots tables, and Quran goal_type/
    // longest_streak/last_*_date columns), 0023 (PR #70 review round
    // 8, item 6: migration_source_ledger's created/reconciled-requires-
    // target_id CHECK constraint and its target-attribution-uniqueness
    // partial unique index), 0024 (PR #70 review round 9, item 4:
    // unifies the 'failed'-row policy by dropping and re-creating 0023's
    // own partial unique index to also cover status='failed'), and 0025
    // (Booking-First Enrollment: enrollments admin/booking-ref columns +
    // submit_enrollment_booking() RPC).
    //
    // Previously hardcoded as a literal number that had to be bumped by
    // hand every time a migration was added (and drifted out of sync when
    // 0025 was added, despite this comment already promising otherwise) —
    // now read directly from the real journal so this assertion tracks the
    // folder's actual size on its own.
    const realJournal = JSON.parse(fs.readFileSync(path.join(realMigrationsFolder, "meta", "_journal.json"), "utf8"));
    const expected = realJournal.entries.length;
    const applied = await pool.query(`select count(*)::int as n from drizzle.__drizzle_migrations;`);
    assert(applied.rows[0].n === expected, `expected all ${expected} migrations tracked as applied (0000-0003 recognized by journal timestamp, not hash + 0004+ newly run), got ${applied.rows[0].n}`);
  });
  ...
  // -------------------------------------------------------------------
  await test("phase 6 (schema): exactly 40 tables in public", async () => {
    // 20-table baseline + 16 added by Stage 2F (0012-0015) + 1 added by
    // 0016 (parent_student_links) + 2 added by Stage 2J-B / 0022
    // (migration_source_ledger, payment_source_snapshots) + 1 added by the
    // scope-correction pass's corrective revision (0029_email_outbox) —
    // see schema.local.test.mjs's matching test for the full name list.
    ...
    assert(rows[0].n === 40, `expected 40 tables, got ${rows[0].n}`);
  });
