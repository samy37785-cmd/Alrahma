...
// them.
//
// IsnadChain hadith citation update (2026-09-19, Mahmoud-approved wording):
// ISNAD_CHAIN_TEXT.ar.quote/citation now carry real Arabic text and a
// specific reference (Sahih al-Bukhari 5027) instead of the previous
// English-only placeholder -- see describe block 2c below. This is a
// regression guard on the approved wording, not an automated proof of
// Islamic-scholarly correctness.
//
// Scope note (this branch only): the upstream `translationStatus.js`
// registry and its describe-block-3 assertions are a separate, not-yet-
// approved phase and are intentionally not part of this branch's file list
// -- describe block 3 below is trimmed to what this file's own modules can
// prove without importing that registry.
...
// IsnadChain hadith citation, Mahmoud-approved wording (2026-09-19). This
// block is a regression guard on the specific, approved English/Arabic
// text and reference below -- it is NOT an automated proof of Islamic-
// scholarly correctness, a fatwa, or an independent legal review, and must
// not be read as one.
describe('2c. IsnadChain hadith citation (Mahmoud-approved, 2026-09-19): Sahih al-Bukhari 5027', () => {
  it('English quote is the approved wording, unchanged', () => {
    expect(ISNAD_CHAIN_TEXT.en.quote).toBe('"The best of you are those who learn the Quran and teach it."');
  });

  it('English citation names the approved specific reference (Sahih al-Bukhari 5027)', () => {
    expect(ISNAD_CHAIN_TEXT.en.citation).toBe('— Sahih al-Bukhari 5027');
    expect(ISNAD_CHAIN_TEXT.en.citation).toContain('5027');
  });

  it('Arabic quote is the approved Arabic wording, exactly', () => {
    expect(ISNAD_CHAIN_TEXT.ar.quote).toBe('خَيْرُكُمْ مَنْ تَعَلَّمَ الْقُرْآنَ وَعَلَّمَهُ');
  });

  it('Arabic citation is the approved reference, exactly, and carries the Arabic-numeral hadith number (٥٠٢٧)', () => {
    expect(ISNAD_CHAIN_TEXT.ar.citation).toBe('— صحيح البخاري ٥٠٢٧');
    expect(ISNAD_CHAIN_TEXT.ar.citation).toContain('٥٠٢٧');
  });

  it('the Arabic quote is real Arabic text, not the English string (the previous English-only placeholder is gone)', () => {
    expect(ISNAD_CHAIN_TEXT.ar.quote).not.toBe(ISNAD_CHAIN_TEXT.en.quote);
    expect(ISNAD_CHAIN_TEXT.ar.quote).not.toMatch(/[A-Za-z]{3,}/);
  });

  it('no other religious/structural text in ISNAD_CHAIN_TEXT changed as a side effect of this citation update', () => {
    expect(ISNAD_CHAIN_TEXT.en.eyebrow).toBe('Our Legacy');
    expect(ISNAD_CHAIN_TEXT.en.headingLine1).toBe('Every lesson is connected to');
    expect(ISNAD_CHAIN_TEXT.en.headingLine2).toBe('1,400 years of unbroken transmission');
    expect(ISNAD_CHAIN_TEXT.en.nodes.prophet.detail).toBe('Received revelation in the Cave of Hira');
    expect(ISNAD_CHAIN_TEXT.en.ctaGift).toBe('Give your child this gift →');
    expect(ISNAD_CHAIN_TEXT.en.ctaMeet).toBe('Meet our Ijazah holders');
    expect(ISNAD_CHAIN_TEXT.ar.eyebrow).toBe('إرثنا');
    expect(ISNAD_CHAIN_TEXT.ar.nodes.prophet.detail).toBe('تلقّى الوحي في غار حراء');
    expect(ISNAD_CHAIN_TEXT.ar.ctaGift).toBe('امنح طفلك هذه الهدية ←');
    expect(ISNAD_CHAIN_TEXT.ar.ctaMeet).toBe('تعرّف على حاملي الإجازة لدينا');
  });
});
