...
async function main() {
  ...
  // -------------------------------------------------------------------
  await test("public schema has exactly 39 tables (auth.users excluded)", async () => {
    ...
    // both new tables, no existing table dropped.
    ...
    assert(
      rows.length === 39,
      `expected 39 public tables, got ${rows.length}: ${rows.map((r) => r.table_name).join(", ")}`,
    );
  });
