...
//
// Standalone (not wired into orchestrate-db-tests.mjs's fixed 5-suite
// DB_ASSERTION_CONTRACT — same posture as rpc-admin-booking-activation.
// local.test.mjs and enrollment-status-reconciliation.local.test.mjs).
// Run directly:
...
async function main() {
  ...
  const harnessOk = harness.report();
  ...
  if (dbFailed.length > 0 || !harnessOk) process.exitCode = 1;
}
