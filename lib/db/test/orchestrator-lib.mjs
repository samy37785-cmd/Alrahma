...
export const DB_ASSERTION_CONTRACT = [
  ...
  // Stage 2J-B / 0022_lossless_migration_support.sql raised these three:
  // schema.local.test.mjs 67->68 (37->39 public-table count assertion
  // stays 1 test; the payments-guest-checkout test was split in two),
  // rls-full-matrix.local.test.mjs 61->70 (+9: guest-payment visibility +
  // migration_source_ledger/payment_source_snapshots RLS coverage),
  // acl.local.test.mjs 18->19 (+1: the same two new tables' GRANT-layer
  // coverage). rls.local.test.mjs and upgrade-scenario.local.test.mjs
  // stayed unchanged then (upgrade-scenario's migration-count assertion
  // inside an existing test moved 22->23, not a new test).
  //
  // Booking-First Enrollment / 0025_booking_first_enrollment.sql raised
  // two more: rls.local.test.mjs 71->76 (+5: submit_enrollment_booking()
  // RPC — real booking_ref returned, status forced to 'new', no financial
  // field settable, collision-free across two calls, whatsapp required;
  // plus a non-admin authenticated raw UPDATE forgery attempt affecting 0
  // rows), acl.local.test.mjs 19->20 (+1: anon has no column-level INSERT
  // grant at all on any of the 7 new admin/financial columns — one test,
  // looping over all 7 columns, same granularity as this file's other
  // multi-column checks).
  //
  // STOP GATE C / 0026_live_classes_nullable_teacher.sql raised two more:
  // schema.local.test.mjs 68->69 (+1: live_classes.teacher_id is
  // genuinely nullable at the information_schema level, not just accepted
  // by app-layer validation), rls.local.test.mjs 76->80 (+4: an AAL2 admin
  // can insert teacher_id = NULL; the assigned student can still read
  // that row via the pre-existing, untouched student_id = auth.uid()
  // SELECT branch; an ordinary non-admin authenticated caller is still
  // refused a NULL-teacher insert; a real teacher_id still both works and
  // remains genuinely FK-enforced — 0026 only dropped NOT NULL, not the
  // foreign key). Real, disposable-Postgres assertions — mocked
  // controller tests are not sufficient proof for a schema/RLS change.
  //
  // Auth hardening security batch (item 6c) / 0027_review_relationship_
  // check.sql raised one more: rls.local.test.mjs 80->86 (+6: seed
  // fixture; a student with course_progress for a course can review it; a
  // student with none is rejected by reviews_insert_own; a student can
  // review their own assigned teacher via my_teacher_id(); a student
  // reviewing a teacher who isn't their own is rejected; setting both
  // course_id and teacher_id on one row — a bypass attempt smuggling an
  // owned relationship alongside an unowned one — is still rejected).
  //
  // Full production cutover, Phase 0 item 5 / 0033_profiles_token_version
  // .sql raised one more: rls.local.test.mjs 86->88 (+2: bump_token_
  // version() only ever increments the calling user's own token_version,
  // starting from 0, and never another user's; an unauthenticated caller
  // is rejected) — closes the real Supabase-mode session-invalidation gap
  // config/validateEnv.js used to hard-block production boot on.
  //
  // Production-readiness audit follow-up (2026-09-17) raised one more:
  // rls.local.test.mjs 88->93 (+5: proves, with real RLS assertions, that
  // backend/data/supabase/routes/liveClassRoutes.js's customer-facing
  // POST/PATCH/DELETE /api/classes — gated only by `protect`, no staff/
  // role check in the route file itself — is not exploitable, because
  // live_classes_insert_teacher_or_admin/_update_owner_or_admin/_delete_
  // owner_or_admin (0015) are the real authorization boundary: the
  // genuine assigned teacher can create/update their student's class; a
  // plain authenticated user who is not that student's teacher is
  // rejected on INSERT and silently affects 0 rows on UPDATE/DELETE).
  //
  // Production-readiness audit follow-up (2026-09-17) also raised
  // rls-full-matrix.local.test.mjs 70->73 (+3: increment_blog_view()
  // closes the Supabase-mode blog-view-count gap — anon can increment a
  // published post exactly once per call; a draft/nonexistent slug
  // returns null, no error, no row changed).
  { name: "schema.local.test.mjs", expectedTotal: 69 },
  { name: "rls.local.test.mjs", expectedTotal: 93 },
  { name: "rls-full-matrix.local.test.mjs", expectedTotal: 73 },
  { name: "acl.local.test.mjs", expectedTotal: 20 },
  { name: "upgrade-scenario.local.test.mjs", expectedTotal: 9 },
  // Production-readiness audit follow-up (2026-09-17): these three existed
  // as real, permanent test files but were never wired into this pipeline
  // (each had its own "standalone, not wired into DB_ASSERTION_CONTRACT"
  // comment) — meaning `npm run test:db` and CI never actually ran them,
  // including the one suite proving the newest capability this cutover
  // effort added (admin-approve -> activate-subscription RPC). Now real
  // pipeline steps like every other suite above.
  { name: "rpc-admin-booking-activation.local.test.mjs", expectedTotal: 10 },
  { name: "enrollment-status-reconciliation.local.test.mjs", expectedTotal: 11 },
  { name: "enrollment-new-to-pending-migration.local.test.mjs", expectedTotal: 10 },
];
