...
This document's own findings above are accurate for the state of the code as it stood when written, and are **not rewritten**. A follow-up task ("Stage 2C Final Corrective") closed three gaps this document explicitly left open rather than resolved: the `AdminUsersTab.jsx` family-name gate, the `AdminDashboard.jsx` KPI/Teachers-list card, and the `AdminClassesTab.jsx` picker filter — all three listed above as UNKNOWN/reviewed-and-kept/out-of-scope — were each investigated and resolved (two deleted as orphans, one fixed to use the real `users` list with no role filter). It also applied `normalizeAccountRole()` at AuthContext's real data boundaries (previously documented as the contract but never actually called) and redesigned `isVerifiedAdminSession()`/`AdminAuthContext` to be fail-closed (previously `Boolean(adminUser)`, letting a cached-but-unverified profile alone render the Admin shell). Two small factual errors in this document's own bookkeeping (a test-count miscount, and an "the two" vs. actually-three-item summary undercount) are also corrected there. Full detail: `docs/user-admin-auth-contract.md`, Appendix B.
