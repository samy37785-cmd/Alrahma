...
async function main() {
  ...
  await test("all 8 hand-authored functions exist", async () => {
    // Baseline remediation: was "all 6" while actually checking 7 names
    // (a real accuracy bug). Now correctly reflects the post-remediation
    // set: forbid_refund_of_refund() was replaced by
    // validate_refund_insert(); claim_provider_event() was split into
    // claim_provider_event() + complete_provider_event().
    const expected = [
      "handle_new_user", "enforce_payment_status_transition", "forbid_payment_delete",
      "validate_refund_insert", "forbid_audit_log_mutation", "claim_provider_event",
      "complete_provider_event", "set_updated_at",
    ];
    const { rows } = await pool.query(`
  ...
  await test("complete_provider_event() finalizes a claimed event, and a repeat complete is a no-op", async () => {
    ...
    const eventId = rows[0].id;
    await pool.query(`select * from public.claim_provider_event($1);`, [eventId]);

    const done = await pool.query(`select * from public.complete_provider_event($1, 'processed', null);`, [eventId]);
    assert(done.rows.length === 1 && done.rows[0].processing_status === "processed");

    const repeat = await pool.query(`select * from public.complete_provider_event($1, 'processed', null);`, [eventId]);
    assert(repeat.rows.length === 0, "completing an already-processed event again should be a no-op");
  });

  await test("complete_provider_event() cannot finalize an event that was never claimed", async () => {
    const { rows } = await pool.query(
    ...
    const eventId = rows[0].id;
    const res = await pool.query(`select * from public.complete_provider_event($1, 'processed', null);`, [eventId]);
    assert(res.rows.length === 0, "completing a still-pending (never claimed) event should return 0 rows");
  });
