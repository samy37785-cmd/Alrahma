...
async function main() {
  ...
  // -------------------------------------------------------------------
  await test("phase 3: apply the full migrations folder — only 0004-0011 actually run", async () => {
    ...
    assert(applied.rows[0].n === 12, `expected all 12 migrations tracked as applied (0000-0003 recognized by journal timestamp, not hash + 0004-0011 newly run), got ${applied.rows[0].n}`);
  });
