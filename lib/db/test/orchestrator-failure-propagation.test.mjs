...
// `finally`. It does NOT count toward the 241-assertion DB suite total
...
async function main() {
  ...
  // -----------------------------------------------------------------
  await test("contract: the fully correct 241-assertion set satisfies the contract", async () => {
    const steps = [
      ...goldenSteps(),
  ...
  await test("contract: a suite reporting the wrong total (e.g. 1/1 instead of its real expected count) fails the gate", async () => {
    const steps = goldenSteps();
    const victim = steps.find((s) => s.name === "acl.local.test.mjs"); // really expects 19
    victim.passed = 1;
    victim.total = 1;
    const result = evaluateAssertionContract(steps);
    assert(result.ok === false, "expected ok=false for a suite reporting the wrong total");
    assert(
      result.problems.some((p) => p.includes("acl.local.test.mjs") && p.includes("reported total 1, expected exactly 19")),
      `expected a wrong-total problem for acl.local.test.mjs, got: ${JSON.stringify(result.problems)}`,
    );
  });

  await test("contract: aggregate below 241 fails the gate even though every PRESENT suite has passed === total", async () => {
