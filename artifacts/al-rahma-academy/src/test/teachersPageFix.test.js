...
// Small fix (2026-09-18), NOT a Phase 2b/2c-style migration: /academy/teachers'
// page shell and per-teacher data were ALREADY genuinely 6-language; this
// closes the three remaining isAr forks in Teachers.jsx itself (breadcrumb
// "Academy" label, SEO title/description, language-filter "All" option) and
// then proves the whole route is genuinely publishable, promoting it from
// 'legacy' to 'published' for ar/it/es/de/fr with real evidence.
...
describe('/academy/teachers promoted to published on real evidence', () => {
  it('ar/it/es/de/fr are all published in the registry (not legacy)', () => {
    for (const lang of ['ar', 'it', 'es', 'de', 'fr']) {
      expect(TRANSLATION_STATUS['/academy/teachers'].languages[lang].status).toBe('published');
      expect(isPublished('/academy/teachers', lang)).toBe(true);
    }
  });

  it('en is (still, implicitly) published for every language code LANGS knows about', () => {
    for (const lang of LANGS) expect(isPublished('/academy/teachers', 'en')).toBe(true);
  });

  it('evidence #1: i18nParity.test.js structurally covers teachersPg -- spot check that en/ar/it/es/de/fr all expose the identical teachersPg key set', () => {
  ...
  it('evidence #2: every teacher and every shared credential genuinely has all 6 languages (title/bio/specialties/label)', () => {
