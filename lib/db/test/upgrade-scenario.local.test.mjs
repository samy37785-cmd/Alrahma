...
async function main() {
  ...
  // -------------------------------------------------------------------
  await test("phase 3: apply the full migrations folder — only 0004-0019 actually run", async () => {
    ...
    // subscription-grant RPC), 0018 (admin subrouter closure: profiles.
    // is_teacher + admin_set_teacher_flag/admin_assign_teacher/admin_set_
    // family_name/admin_deactivate_subscription/admin_update_profile +
    // enrollments admin insert policy), and 0019 (teachers_public view, for
    // the /api/search route-parity gap) — 20 migrations total in the folder
    // now, up from 19. This count tracks the folder's real size, not a
    // number to keep in sync by hand every time a migration is added.
    const expected = 20;
    const applied = await pool.query(`select count(*)::int as n from drizzle.__drizzle_migrations;`);
    assert(applied.rows[0].n === expected, `expected all ${expected} migrations tracked as applied (0000-0003 recognized by journal timestamp, not hash + 0004-0019 newly run), got ${applied.rows[0].n}`);
  });
