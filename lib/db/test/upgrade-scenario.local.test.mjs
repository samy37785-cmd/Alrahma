...
async function main() {
  ...
  // -------------------------------------------------------------------
  await test("phase 3: apply the full migrations folder — only 0004-0022 actually run", async () => {
    ...
    // rehearsal — see docs/option-a-mongo-supabase-parity-map.md), and 0022
    ...
    // longest_streak/last_*_date columns) — 23 migrations total in the
    // folder now, up from 22. This count tracks the folder's real size,
    // not a number to keep in sync by hand every time a migration is
    // added.
    const expected = 23;
